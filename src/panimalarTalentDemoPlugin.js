...
function mockPanimalarStudents(role) {
  ...
  return firstNames.map((first, i) => {
    ...
    return { id: `panimalar-demo-${i + 1}`, name, email: `student${i + 1}@panimalar.ac.in`, college: PANIMALAR, department: departments[i % departments.length], graduationYear: i % 4 === 0 ? 2026 : 2027, passedOut: false, suitability: score, verification_score: Math.max(60, score - 2), matchingSkills: matched, missingSkills: skills.slice(matched.length), careers: [role, i % 2 ? "Full Stack Developer" : "Frontend Developer"] };
  });
}

const demoSource = `\nconst PANIMALAR = "Panimalar Engineering College";\nconst firstNames = ${JSON.stringify(firstNames)};\nconst lastNames = ${JSON.stringify(lastNames)};\nconst departments = ${JSON.stringify(departments)};\nconst roleSkills = ${JSON.stringify(roleSkills)};\nfunction mockPanimalarStudents(role) {\n  const skills = roleSkills[role] || roleSkills["Full Stack Developer"];\n  return firstNames.map((first, i) => {\n    const score = Math.max(62, 94 - i);\n    const matched = skills.slice(0, Math.max(2, skills.length - (i % 3))).map((skill, j) => ({ name: skill, score: Math.max(60, score - j * 3) }));\n    return { id: \"panimalar-demo-\" + (i + 1), name: first + \" \" + lastNames[i % lastNames.length], email: \"student\" + (i + 1) + \"@panimalar.ac.in\", college: PANIMALAR, department: departments[i % departments.length], graduationYear: i % 4 === 0 ? 2026 : 2027, passedOut: false, suitability: score, verification_score: Math.max(60, score - 2), matchingSkills: matched, missingSkills: skills.slice(matched.length), careers: [role, i % 2 ? \"Full Stack Developer\" : \"Frontend Developer\"] };\n  });\n}\n`;
