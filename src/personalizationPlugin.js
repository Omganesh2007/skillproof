...
export function personalizationPlugin() {
  return {
    ...
    async transform(code, id) {
      ...
      if (!next.includes("function WelcomeBanner")) {
        next = next.replace(/\nfunction App\(/, "\n" + welcome + "\nfunction App(");
      }
      ...
      if (appShellPattern.test(next) && !next.includes("<WelcomeBanner student={student} />")) {
        next = next.replace(appShellPattern, (match, args, body, ret) => `function AppShell(${args}) {${body}${ret} className="min-h-screen bg-slate-50"><WelcomeBanner student={student} />`);
      }
