...
async function searchTalent(role) {
  ...
  if (!token) throw new Error("Please sign in again as Industry to search talent.");
  const response = await fetch(`${API_BASE}/industry/talent?role=${encodeURIComponent(role)}`, {
  ...
  const students = (data.students || []).map((s) => ({
    ...
    passedOut: Boolean(s.passedOut ?? s.passed_out),
    verification_score: Number(s.verification_score ?? s.readiness ?? 0),
    matchingSkills: s.matchingSkills || s.matching_skills || [],
    missingSkills: (s.missingSkills || s.missing_skills || []).map((x) => typeof x === "string" ? x : x.name),
  }));
  const colleges = (data.colleges || []).map((group) => {
    const groupStudents = students.filter((s) => (s.college || "College not provided") === group.name);
    return {
      college: group.name,
      students: groupStudents,
      studentCount: groupStudents.length,
      topSuitability: groupStudents.length ? Math.max(...groupStudents.map((s) => Number(s.suitability || 0))) : 0,
    };
  });
  return {
    role: data.role,
    requiredSkills: Object.keys(data.requirements || {}),
    totalStudents: Number(data.count ?? students.length),
    students,
    colleges,
  };
}

function passedOutLabel(student) { return student.passedOut ? "Passed out" : "Not passed out"; }
function suitabilityTone(score) { if (score >= 80) return "text-teal-700 bg-teal-50 border-teal-100"; if (score >= 60) return "text-amber-700 bg-amber-50 border-amber-100"; return "text-slate-600 bg-slate-50 border-slate-200"; }
...
function StudentResult({ student, onOpen }) {
  return <button type="button" onClick={() => onOpen(student)} className="w-full text-left bg-white border border-slate-200 rounded-2xl p-5 hover:border-teal-300 hover:shadow-sm transition">
    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4"><div className="min-w-0"><div className="flex items-center gap-3"><h3 className="font-bold text-lg truncate">{student.name}</h3><span className="text-xs px-2.5 py-1 rounded-full border border-slate-200 text-slate-500">{student.department || "Department not provided"}</span></div><p className="text-sm text-slate-500 mt-1">{student.college || "College not provided"}</p><div className="flex flex-wrap gap-2 mt-3">{(student.matchingSkills || []).slice(0, 5).map((skill) => <span key={skill.name} className="text-xs px-2.5 py-1 rounded-full bg-teal-50 text-teal-700 border border-teal-100">✓ {skill.name} {skill.score}%</span>)}</div></div><div className="lg:text-right shrink-0"><p className="text-xs text-slate-400">Role suitability</p><p className="text-3xl font-bold text-teal-700">{student.suitability}%</p><p className="text-xs text-slate-500 mt-1">{passedOutLabel(student)} · {student.graduationYear || "Year not provided"}</p><p className="text-xs text-slate-400 mt-1">{student.email || "Email not provided"}</p></div></div>
  </button>;
}

function StudentDetail({ student, onClose }) {
  if (!student) return null;
  return <div className="fixed inset-0 z-50 bg-slate-900/30 flex items-center justify-center p-4" onClick={onClose}><div className="w-full max-w-2xl max-h-[90vh] overflow-auto bg-white rounded-3xl border border-slate-200 shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
...
export function IndustryDashboard({ logout }) {
  const [page, setPage] = useState("dashboard"); const [role, setRole] = useState(ROLES[0]); const [roleQuery, setRoleQuery] = useState(""); const [results, setResults] = useState(null); const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const [openCollege, setOpenCollege] = useState(""); const [selectedStudent, setSelectedStudent] = useState(null);
  const user = useMemo(() => { try { return JSON.parse(localStorage.getItem("skillproof_industry_user") || "{}"); } catch { return {}; } }, []);
  const company = user.company || user.organization || user.name || "Industry Partner"; const email = user.email || "Not provided"; const department = user.department || "Talent Acquisition"; const initials = company.split(/\s+/).map((x) => x[0]).join("").slice(0, 2).toUpperCase() || "IP"; const filteredRoles = ROLES.filter((item) => item.toLowerCase().includes(roleQuery.toLowerCase()));
  const runSearch = async () => { if (!role) return; setLoading(true); setError(""); setOpenCollege(""); try { setResults(await searchTalent(role)); } catch (err) { setResults(null); setError(err.message || "Unable to search talent"); } finally { setLoading(false); } };
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900"><aside className="fixed inset-y-0 left-0 w-64 bg-white border-r border-slate-200"><div className="p-5 border-b border-slate-100"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center font-bold">S</div><div><p className="font-bold">SkillProof</p><p className="text-xs text-slate-400">Industry</p></div></div></div><nav className="p-4 space-y-2">{nav.map(([id, label, Icon]) => <button key={id} onClick={() => setPage(id)} className={`w-full h-11 rounded-xl px-3 flex items-center gap-3 text-sm font-medium ${page === id ? "bg-slate-100 text-slate-900" : "text-slate-500 hover:bg-slate-50"}`}><Icon size={17}/>{label}</button>)}</nav><div className="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-100"><button onClick={logout} className="w-full h-10 flex items-center gap-2 px-2 text-sm text-slate-500"><LogOut size={16}/>Sign out</button></div></aside>
